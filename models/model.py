...
class WordEmbedding(nn.Module):
    def __init__(self, vocab_size: int) -> None:
        ...
        self.embedding: nn.Embedding = nn.Embedding(vocab_size, settings.D_MODEL)
        
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # (batch, seq_len, 1) --> (batch, seq_len, d_model)
        return self.embedding.forward(x) * math.sqrt(settings.D_MODEL)
...
class PositionalEncoding(nn.Module):
    def __init__(self) -> None:
        ...
        # (seq_len, d_model)
        pe = torch.zeros(settings.SEQ_LEN, settings.D_MODEL)
        
        # (seq_len, 1)
        pos = torch.arange(0, settings.SEQ_LEN, dtype=torch.float).float().unsqueeze(1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor: 
        assert x.shape[1] <= settings.SEQ_LEN, f"Input sequence length exceeds the position encoder's max sequence length  `{settings.SEQ_LEN}`"
...
class FeedForwardBlock(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(settings.D_MODEL, settings.DFF).to(settings.DEVICE)
        self.dropout = nn.Dropout(settings.DROPOUT)
        self.linear_2 = nn.Linear(settings.DFF, settings.D_MODEL).to(settings.DEVICE)
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self) -> None:
        assert settings.D_MODEL % settings.HEADS == 0, "d_model is not divisible by heads"
        
        super().__init__()
        
        self.d_k = settings.D_MODEL // settings.HEADS
        
        self.W_q = nn.Linear(settings.D_MODEL, settings.D_MODEL, bias=False).to(settings.DEVICE)
        self.W_k = nn.Linear(settings.D_MODEL, settings.D_MODEL, bias=False).to(settings.DEVICE)
        self.W_v = nn.Linear(settings.D_MODEL, settings.D_MODEL, bias=False).to(settings.DEVICE)

        self.W_o = nn.Linear(settings.D_MODEL, settings.D_MODEL, bias=False).to(settings.DEVICE)
        self.dropout = nn.Dropout(settings.DROPOUT)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        query = query.view(query.shape[0], query.shape[1], settings.HEADS, self.d_k).transpose(1, 2)
        key = key.view(key.shape[0], key.shape[1], settings.HEADS, self.d_k).transpose(1, 2)
        value = value.view(value.shape[0], value.shape[1], settings.HEADS, self.d_k).transpose(1, 2)
        ...
        x = x.contiguous().view(x.shape[0], -1, settings.HEADS * self.d_k)
...
class ResidualConnection(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.dropout = nn.Dropout(settings.DROPOUT)
        self.norm = nn.LayerNorm(settings.D_MODEL, device=settings.DEVICE)
...
class ProjectionLayer(nn.Module):
    def __init__(self, vocab_size: int) -> None:
        super().__init__()
        self.proj = nn.Linear(settings.D_MODEL, vocab_size)
...
class MtTransformerModel(nn.Module):
    ...
    def build(
        ...
        # Create settings.N_BLOCKS number of encoders
        encoder_blocks = []
        for _ in range(settings.N_BLOCKS):
        ...
        # Create settings.N_BLOCKS number of decoders
        decoder_blocks = []
        for _ in range(settings.N_BLOCKS):
            self_attention_block = MultiHeadAttentionBlock()
